...
const sumaN = (arrayParam) => arrayParam.reduce((acum, n) => acum+n, 0);
console.log(sumaN(array1));

//45

//A partir d'un array d'strings i numeros, mostra un string que els continga tots
let array2 = ['a', 1, 'b', 2, 'c', 3];
const hiperString = (arrayParam) => arrayParam.reduce((acum, n) => acum+n, "");
console.log(hiperString(array2));
// a1b2c3
...
const dias = (arrayParam) => arrayParam.reduce((objeto, n, i) => {
        objeto[n]=i;
        return objeto;
},{});
console.log(dias(dies));
...
// Usando reduce para devolver los nombres
const components = (arrayParam) => arrayParam.reduce((arrayDevuelto, n) => {
    arrayDevuelto.push(n.nom);
    return arrayDevuelto;
}, []);
console.log(components(musics));

const componentesGuitar = musics.map((el) => {
    let total="";
    if (el.instrument==='guitarra') { total+=el.nom}
    return total;
});
console.log(componentesGuitar);
...
//['Pep', 'Vicent Lloret']
const compsGuitarra = (arrayParam) => arrayParam.reduce((arrayDevuelto, n, i ) => {
    if (n.instrument==='guitarra') { arrayDevuelto.push(n.nom)}
    return arrayDevuelto;
}, []);
console.log(compsGuitarra(musics));
...
const RetornaObjetos = (objetoParam) => objetoParam.reduce((objetoDevuelto, n) => {
    let kk = {};
    kk.email = n.email;
    kk.website = n.website;
    kk.phone = n.phone;
    objetoDevuelto.push(kk);
    return objetoDevuelto;
}, []);
console.log(RetornaObjetos(users));
